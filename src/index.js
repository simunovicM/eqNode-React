...
const fromObject = function (obj) {
  var fnc = (node, children) => {
    if (children && children.length) {
      children.forEach(f => {
        var it = node.addChild(f);
        fnc(it, f.children);
      });
    }
  };
  var node = new Node(obj);
  fnc(node, obj.children);
  return node;
}
...
var Node = function (item) {
  ...
  this.addChild = function (child) {
    var nodeChild = child instanceof Node ? child : new Node(child);
    nodeChild.getParent = function () { return self; };
